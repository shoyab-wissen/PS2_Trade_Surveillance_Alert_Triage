...
_MOCK_VERDICTS = {
    ...
    "MARKING_CLOSE": {
        "verdict": "ESCALATE",
        "confidence": 0.82,
        "false_positive_probability": 0.14,
        "rationale": (
...
class ClaudeTriageClient:
    ...
    def __init__(self):
        ...
        if self._mock_mode:
            print(
                "  [Claude] No valid API key - running in MOCK mode (realistic pre-computed verdicts)"
            )
    ...
    def _mock_triage(self, alert: Alert, profile=None) -> TriageResult:
        ...
        if (
            ...
            mock = _MOCK_VERDICTS.get(
                alert.pattern_type,
                {
                    ...
                    "key_factors": [
                        f"z_score +{alert.z_score:.1f}sigma vs trader baseline",
                        "Pattern thresholds met",
                    ],
        ...
        self.total_cost_usd += self._compute_cost(2100, 280, 1850)
        return TriageResult(
    ...
    def _cached_system(self) -> list:
        ...
        if self.feedback_examples:
            ...
            for ex in self.feedback_examples[
                -3:
            ]:  # keep last 3 to control token growth
                correct_label = (
                    "CONFIRMED CORRECT" if ex.get("correct") else "ANALYST CORRECTION"
                )
    ...
    def _parse_triage_json(self, text: str, alert_id: str) -> TriageResult:
        ...
        return TriageResult(
            ...
            false_positive_probability=float(
                data.get("false_positive_probability", 0.5)
            ),
    ...
    def triage_single(
        ...
        try:
            ...
            print(
                f"  [Claude] API auth failed ({e.status_code}) - switching to mock mode"
            )
            self._mock_mode = True
            return self._mock_triage(alert, profile)
        ...
        result = self._parse_triage_json(
            self._get_response_text(response), alert.alert_id
        )
    ...
    def triage_batch(
        ...
        user_prompt = build_batch_user_prompt(
            alerts, profiles, prior_counts, watchlist_set
        )
        assert self.client is not None
        response = self.client.messages.create(
            model=self.MODEL,
    ...
    def triage_all(
        ...
        for i in range(0, len(medium_low), 5):
            ...
            results.extend(
                self.triage_batch(batch, batch_profiles, prior_counts, watchlist_set)
            )
    ...
    def investigate_trader(
        ...
        if self._mock_mode or not alerts:
            return {
                ...
                "escalation_recommendation": "ELEVATED"
                if len(alerts) >= 2
                else "STANDARD",
                ...
                "regulatory_flags": ["multiple_pattern_types"]
                if len(alerts) >= 2
                else [],
        ...
        text = (
            re.sub(r"```(?:json)?", "", self._get_response_text(response))
            .strip()
            .rstrip("`")
            .strip()
        )
        ...
        result.setdefault("coordinated_confidence", result.get("confidence", 0.65))
        result.setdefault("combined_risk_score", result.get("risk_score", result.get("coordinated_confidence", 0.65)))
        result.setdefault("scheme_type", result.get("verdict", "REVIEW"))
        result.setdefault("escalation_recommendation", "ELEVATED" if len(alerts) >= 2 else "STANDARD")
        result.setdefault("cross_alert_rationale", result.get("rationale", result.get("summary", "")))
    ...
    def natural_language_query(self, question: str) -> dict:
        ...
        if self._mock_mode:
            ...
            return {
                "severity": "HIGH"
                if "high" in q
                else ("critical" if "critical" in q else None),
                "pattern_type": next(
                    (
                        p
                        for p in available_patterns
                        if p.lower().replace("_", " ") in q or p.lower() in q
                    ),
                    None,
                ),
                "verdict": "ESCALATE"
                if "escalat" in q
                else ("DISMISS" if "dismiss" in q else None),
        ...
        user_prompt = build_nl_query_prompt(
            question, available_patterns, available_severities
        )
        assert self.client is not None
        try:
            response = self.client.messages.create(
                ...
                system=[
                    {
                        "type": "text",
                        "text": "You are a query parser. Return only valid JSON.",
                    }
                ],
                messages=[{"role": "user", "content": user_prompt}],
            )
        ...
        try:
            ...
            m = re.search(r"\{.*\}", text, re.DOTALL)
            return (
                json.loads(m.group()) if m else {"error": "parse_failed", "raw": text}
            )
